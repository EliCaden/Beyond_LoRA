...
def _maybe_download_terrain(root: str, url: Optional[str]) -> str:
    if os.path.isdir(root) and any(os.path.isdir(os.path.join(root, d)) for d in os.listdir(root)):
        return root

    url = url or os.environ.get("TERRAINCOGNITA_URL") or os.environ.get("TERRAING_URL") or os.environ.get("TERRA_INCOGNITA_URL")
    if not url:
        return root

    print(f"[TerraIncognita] No data found under {root}. Attempting download: {url}")
...
def _scan_terrain(root: str, domains: Optional[List[str]]) -> List[Tuple[str, str]]:
...
def _build_transforms(
    ...
    *,
    ...
    if train:
        if fast_transform:
            t.append(T.Resize((size, size), interpolation=InterpolationMode.BILINEAR))
...
class _TIImageList(Dataset):
    def __init__(self, items: List[_TIItem], tfm: T.Compose):
        self.items = items
        self.tfm = tfm
...
@register_data("terraincognita")
class TerraIncognitaDataModule(BaseDataModule):
    def __init__(
        self,
        data_root: str,
        batch_size: int = 64,
        ...
        **_: Any,
        ...
        self.train_ds: Optional[_TIImageList] = None
        self.val_ds: Optional[_TIImageList] = None
        self.test_ds: Optional[_TIImageList] = None
    ...
    def setup(self):
        root = self.data_root
        pairs = _scan_terrain(root, self.domains)

        if not pairs and self.download:
            root = _maybe_download_terrain(root, self.download_url)
            pairs = _scan_terrain(root, self.domains)

        if not pairs:
            raise FileNotFoundError(
                f"[TerraIncognita] No images under: {self.data_root}. "
                f"If you want auto-download, pass download=True and provide download_url or set TERRAINCOGNITA_URL."
            )
        ...
        items: List[_TIItem] = [_TIItem(p, cls2id[c]) for p, c in pairs]
        ...
        self.train_ds = _TIImageList(train_items, self.t_train)
        self.val_ds = _TIImageList(val_items, self.t_eval)
        self.test_ds = _TIImageList(test_items, self.t_eval)
