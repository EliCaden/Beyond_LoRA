...
class Flowers102DataModule(BaseDataModule):
    def __init__(
        ...
        super().__init__(data_root=data_root, img_size=img_size, batch_size=batch_size, seed=seed, use_official_splits=use_official_splits, val_split=val_split, test_split=test_split)
    ...
    def setup(self):
        if self.use_official_splits:
            self.train_ds = Flowers102(self.data_root, split="train", download=self.download, transform=self.t_train)
            self.val_ds = Flowers102(self.data_root, split="val", download=self.download, transform=self.t_eval)
            self.test_ds = Flowers102(self.data_root, split="test", download=self.download, transform=self.t_eval)
            return

        base_train = ConcatDataset([
            Flowers102(self.data_root, split="train", download=self.download, transform=self.t_train),
            Flowers102(self.data_root, split="val", download=self.download, transform=self.t_train),
            Flowers102(self.data_root, split="test", download=self.download, transform=self.t_train),
        ])
        base_eval = ConcatDataset([
            Flowers102(self.data_root, split="train", download=self.download, transform=self.t_eval),
            Flowers102(self.data_root, split="val", download=self.download, transform=self.t_eval),
            Flowers102(self.data_root, split="test", download=self.download, transform=self.t_eval),
        ])
